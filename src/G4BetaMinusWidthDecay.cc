...
G4BetaMinusWidthDecay::G4BetaMinusWidthDecay(const G4ParticleDefinition* theParentNucleus,
                                   const G4double& branch, const G4double& e0,
                                   const G4double& excitationE,
                                   const G4Ions::G4FloatLevelBase& flb,
                                   const G4BetaDecayType& betaType)
        : G4NuclearDecay("beta- decay", BetaMinusWidth, excitationE, flb), endpointEnergy(e0)
{
    ...
    widthReadSucces = ReadWidthFile(daughterZ,daughterA,excitationE*MeV, endpointEnergy*MeV);
    if(widthReadSucces){
        SetUpBetaSpectrumSamplers(daughterZ, daughterA, betaType);
    }
...
G4BetaMinusWidthDecay::~G4BetaMinusWidthDecay()
{
    delete spectrumSampler;
}


G4DecayProducts* G4BetaMinusWidthDecay::DecayIt(G4double)
{
    if(widthReadSucces){
        ...
        G4double eKE = sublevelQvalue*spectrumSampler->shoot(G4Random::getTheEngine() );
        G4double eMomentum = std::sqrt(eKE*(eKE + 2.*eMass) );

        G4double cosThetaENu = 2.*G4UniformRand() - 1.;
        G4double eTE = eMass + eKE;
        G4double nuEnergy = ((sublevelQvalue - eKE)*(parentMass + nucleusMass - eTE)
        ...
        /*G4cout << "sublevelQ" << sublevelQvalue << G4endl;
...
void
G4BetaMinusWidthDecay::SetUpBetaSpectrumSampler(const G4int& daughterZ,
...
void
G4BetaMinusWidthDecay::SetUpBetaSpectrumSamplers(const G4int& daughterZ,
                                                const G4int& daughterA,
                                                const G4BetaDecayType& betaType)
{
    for(int i = 0; i < sublevelQvalues.size(); i++){
        G4double e0 = sublevelQvalues[i]/CLHEP::electron_mass_c2;
        G4BetaDecayCorrections corrections(daughterZ, daughterA);
        spectrumSampler = 0;
        ...
        if (e0 > 0) {
            ...
            spectrumSamplers.push_back(new G4RandGeneral(pdf, npti));
            delete[] pdf;
        }
    }
}

void G4BetaMinusWidthDecay::DumpNuclearInfo()
{
